...
impl VM {
    ...
    fn add_push_scalar_instr(&mut self) {
        self.add_instr_with_operand(0x00000020, 
            String::from("OP_PUSHSCALAR"));
    ...
    fn exec_push_scalar_instr(&mut self) {
        let operand = self.instr_mem.as_ref().unwrap()
            .get_instr(self.pc as usize).get_operand();
        let data = self.data_mem.as_ref().unwrap()
            .get_data(operand as usize);
